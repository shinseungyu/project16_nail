...
export default function NailCalculatorWidget() {
  ...
  const level =
    total < 800 ? { label: '소규모 시작', color: 'text-green-500', bg: 'bg-green-50', msg: '공유 작업실 또는 소형 독립샵 수준입니다.' }
    : total < 1500 ? { label: '일반 1인샵', color: 'text-pink-500', bg: 'bg-pink-50', msg: '평균적인 1인 네일샵 창업 규모입니다.' }
    : { label: '풀 인테리어샵', color: 'text-yellow-500', bg: 'bg-yellow-50', msg: '인테리어와 장비에 충분히 투자하는 규모입니다.' }
  ...
  return (
    <section className="mb-14">
      <h2 className="text-2xl font-bold text-gray-900 mb-2">내 창업비용 직접 계산하기</h2>
      <p className="text-sm text-gray-400 mb-6">항목별 슬라이더를 조정해 예상 창업비용을 바로 확인하세요.</p>
              ...
              <div key={label}>
                <div className="flex justify-between mb-1">
                  <div>
                    ...
                    <span className="text-xs text-gray-300 ml-2">{hint}</span>
                  </div>
                  <span className="text-sm font-bold text-pink-500">{value.toLocaleString()}만원</span>
                </div>
                <input
          ...
          <div className="flex flex-col justify-center gap-3">
            {items.map(({ label, value }) => (
              <div key={label} className="flex justify-between items-center py-1.5 border-b border-gray-50">
                ...
                <span className="text-sm font-bold text-gray-700">{value.toLocaleString()}만원</span>
              </div>
            ))}

            <div className={`mt-2 rounded-xl p-5 ${level.bg}`}>
              <div className="flex justify-between items-center mb-2">
                <span className="text-base font-bold text-gray-700">총 창업비용</span>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-gray-400">{level.msg}</span>
              </div>
            </div>

            <a
              href="/calculator"
              className="mt-1 text-center text-sm text-pink-500 font-bold hover:underline"
            >
              수익·손익분기점까지 계산하기 →
            </a>
          </div>
